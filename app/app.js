...
const loadData = async () => {
  ...
  try {
    const [patients, episodesMeta, indexData] = await Promise.all([
      fetchJson(`${DATASET_BASE}/metadata/patients.json`),
      fetchJson(`${DATASET_BASE}/metadata/episodes.json`),
      fetchJson(`${OUTPUT_BASE}/episodes/index.json`),
    ]);

    state.patients = patients;
    state.episodesMeta = episodesMeta;

    const outputs = await Promise.all(
      indexData.episodes.map((file) => fetchJson(`${OUTPUT_BASE}/episodes/${file}`))
    );

    state.outputs = outputs;
    state.outputById = Object.fromEntries(outputs.map((ep) => [ep.episode_id, ep]));

    if (statusEl) statusEl.textContent = "Data Ready";
  } catch (err) {
...
const initDashboard = () => {
  ...
  const renderPatient = (patientId) => {
    ...
    const outputs = episodesMeta
      .map((meta) => ({ meta, output: state.outputById[meta.episode_id] }))
...
const initSummary = () => {
  ...
  const renderSummarySection = (patientId) => {
    const episodesMeta = state.episodesMeta.filter((ep) => ep.patient_id === patientId);
    const outputs = episodesMeta
      .map((meta) => state.outputById[meta.episode_id])
      .filter(Boolean);
    ...
    nextStepsEl.innerHTML = renderList(nextSteps.slice(0, 12));
    timelineHighlights.innerHTML = renderList(highlights.slice(0, 10));
  };
  ...
  if (state.patients.length) {
    renderSummarySection(state.patients[0].patient_id);
  }

  patientSelect.addEventListener("change", (event) => renderSummarySection(event.target.value));
};
...
const initPage = async () => {
  ...
  if (page === "symptom") initSymptom();
  if (page === "legal") initLegal();
  if (page === "medical") initMedical();
};
